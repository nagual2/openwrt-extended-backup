...
- `openwrt_full_backup`
  - Архивирует весь пользовательский слой (`/overlay`) в tar.gz с сохранением прав и владельцев, исключая временные каталоги и спецфайлы.
  - Управляется компактным набором опций: `--output` для каталога назначения, `--overlay` для источника данных, `--dry-run` для проверки без побочных эффектов и `--export=smb` для временного SMB-доступа.
  - По завершении сообщает путь до архива, выводит готовую команду `scp`, а при включённом SMB-экспорте настраивает шару через `ksmbd` и печатает параметры подключения.
...
2. После завершения скрипт сообщит путь к архиву (по умолчанию `/tmp`, переопределяется через `--output`) и выведет готовую команду `scp`.
...
5. (Опционально) Для сетевого доступа по SMB запустите `openwrt_full_backup --export=smb --output /tmp/archive` на устройстве с установленным `ksmbd` — скрипт выведет имя пользователя, пароль и путь к временной шаре.
...
| --- | --- | --- |
| `openwrt_full_backup` | `-h`, `--help`, `-V`, `--version`, `--output PATH`, `--overlay DIR`, `--dry-run`, `--export=smb` | Создаёт архив `/overlay`, по умолчанию сохраняет его в `/tmp`, выводит команду `scp` и при необходимости настраивает временную SMB-шару через `ksmbd`. |
...
- Текущая версия определяется по первому релизному заголовку в `CHANGELOG.md`. При установке через пакет она сохраняется в `/usr/share/openwrt-extended-backup/VERSION`, чтобы скрипты могли выводить корректное значение на роутере.
- Оба скрипта поддерживают флаги `-V`/`--version` для быстрого вывода версии без запуска основной логики.
- Workflow [`Release`](.github/workflows/release.yml) формирует архивы из каталога `scripts/` и сопровождающей документации и публикует их на GitHub Releases.

## Примеры
### SCP и удалённая выгрузка
```sh
openwrt_full_backup --output /tmp
# Скрипт напечатает команду вида:
# scp root@192.168.1.1:/tmp/fullbackup_OpenWrt_24.10.4_2024-10-20_12-30-00.tar.gz <destination>
# На локальной машине выполните команду, заменив <destination> на каталог для сохранения архива.
```

### SMB / ksmbd
```sh
openwrt_full_backup --export=smb --output /tmp/archive
